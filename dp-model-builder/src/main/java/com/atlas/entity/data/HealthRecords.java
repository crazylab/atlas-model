...
public class HealthRecords extends Entity {
    private AvroType avroType;

    public HealthRecords(AvroType avroType) {
    ...
    public AtlasEntity createEntity(String name) {
        ...
        data.setAttribute("totalRecordCount", 100000);
    ...
    private AtlasEntity createAvroSchema(String name) {
        ...
        fields.add(avroField.createEntity("lastCheckupDate", avroType.dateType));
    ...
    private AtlasEntity createPort(String name, AtlasEntity data) {
        AtlasEntity batchPort = Port.createBatchPort(name, "parquet", createAvroSchema(name), data);
